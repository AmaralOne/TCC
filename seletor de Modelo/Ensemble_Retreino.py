...
class Ensemble_Retreino:
    ...
    def Ensembles_predict(self,ensembles, methods, data,tain_data,test_data,intervalo_dos_estocasticos,tamanhoValidacao, result_validation):
        i = 0
        ...
        for m in range(len(result_validation)):
            if result_validation.iloc[m][0] not in methods:
                continue           
            results_v[result_validation.iloc[m][0]] = pd.Series(result_validation.iloc[m][1:])
    
        print(f'teste: {tain_data[len(tain_data)-tamanhoValidacao:]}')
        forecast_errors_mse = self.predict_erros.error_MSE(methods,results_v,tain_data[len(tain_data)-tamanhoValidacao:])      
        forecast_errors_rmse = self.predict_erros.error_RMSE(methods,results_v,tain_data[len(tain_data)-tamanhoValidacao:])
