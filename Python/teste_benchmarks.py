...
def mlp_bench(x_train, y_train, x_test, fh):
    ...
    plt.plot(x_train)
    plt.plot(predct_train)
    plt.title('Série treino')
    plt.show()
...
def main():
    ...
    for j in index:
        #ts = u_M4.getSerieCompleto(j)
        #ts = u_bio.read_Dataset_BIO(j)
        ...
        plt.plot(ts)
        plt.title('Série Aleatoria '+str(j))
        plt.show()
        print('Série Aleatoria '+str(j))
        print(len(ts))
        t1 = time.time()
        # remove seasonality
        seasonality_in = deseasonalize(ts, freq)
        ...
        plt.plot(ts)
        plt.title('Série Aleatoria '+str(j)+" sem sazonalidade")
        plt.show()
        ...
        plt.plot(ts)
        plt.title('Série Aleatoria '+str(j)+" sem tendencia")
        plt.show()
     #   t2 = time.time()
        ...
        #tempoExec = time.time() - t2
        #print("Tempo de execução RNN: {} segundos".format(tempoExec))
        ...
        tempoExec = time.time() - t3
        print("Tempo de execução MLP: {} segundos".format(tempoExec))
      ...
      #  err_RNN_MASE.append(mase(ts[:-fh], y_test, y_hat_test_RNN, freq))
        ...
        plt.plot(y_hat_test_MLP)
        plt.title('Série treino -1')
        plt.show()
        ...
        err_Naive.append(np.mean(abs(insample[freq:] - y_hat_naive)))
        
        tempoExec = time.time() - t1
        print("Tempo de execução total: {} segundos".format(tempoExec))
        # memory handling
        ker.clear_session()
